...
object is automatically saved and calling `ht_shiny()` without the heatmap
...
calling `ht_shiny()` without the heatmap object will convert the static signature heatmap into an
interactive one:
...
Note the functionality of autonatically saving the last heatmap is only truned
...
in `ht_shiny()` which makes it clear which heatmap is becoming interactive.
...
As you see, `heatmap()` function is not exported from **ComplexHeatmap** (so
...
Also `heatmap.2()` is not exported from **ComplexHeatmap** (so it will not
