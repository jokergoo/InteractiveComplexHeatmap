...
boxes. You can check `htShinyExample(4.1)` where the size of the heatmap box is
...
One last example is `htShinyExample(5.4)` where the heatmap visualizes
...
signal. This example can also be run by `htShinyExample(6.2)`.
...
In the next example (`htShinyExample(6.3)`), a different heatmap is generated
