...
ui = fluidPage(
    ...
    plotOutput("heatmap", width = 500, height = 500, click = "heatmap_click", brush = "heatmap_brush"),
...
server = function(input, output, session) {
    ...
    observeEvent(input$heatmap_click, {
        pos = InteractiveComplexHeatmap:::get_pos_from_click(input$heatmap_click)

        selection = selectPosition(ht_obj(), pos, mark = FALSE, ht_pos = ht_pos_obj(), verbose = FALSE)
    ...
    observeEvent(input$heatmap_brush, {
        lt = InteractiveComplexHeatmap:::get_pos_from_brush(input$heatmap_brush)

        selection = selectArea(ht_obj(), lt[[1]], lt[[2]], mark = FALSE, ht_pos = ht_pos_obj(), verbose = FALSE)
...
2. To get the position of the click on heatmap, `InteractiveComplexHeatmap:::get_pos_from_click()` should be used. Note this function is not exported, thus you need
   to add `InteractiveComplexHeatmap:::` prefix. With knowing the position of the click, it can be sent to `selectPosition()` to correspond to the original matrix.
3. Similarly, to get the positions of the area that was brushed on heatmap, `InteractiveComplexHeatmap:::get_pos_from_brush()` should be used.
