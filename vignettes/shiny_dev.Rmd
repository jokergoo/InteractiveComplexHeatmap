...
orignal heatmap, the sub-heatmap and an output of clicking or brushing on the
heatmap. The original heatmap and sub-heatmap components can be resized by
...
With different layout, different default values are assigned to widths and heights of the three components to make
sure they are well arranged.

By default, to get the information of a single cell in the heatmap, a `"click"`
action is used. In `InteractiveComplexHeatmapOutput()`, the action can be set
to `"hover"` or `"dblclick"`. Note when the action
is set to click or hover, when applying the brush action, click or hover will
always be first applied and the output for a single cell appears
and disappears quickly. A "dblclick" has no conflict with hover, but the user experience
is less friendly. Thus, "click" is the default action for getting information 
of a single cell.
...
The last example in `htShinyExample(5.5)` links selected differential genes to 
the MA plot in a DESeq2 analysis, where the MA plot is implemented as the "output".

```{r, echo = FALSE, out.width = "100%"}
include_graphics("https://jokergoo.github.io/images/deseq2.png")
```
...
analysis. There might be three scenarios:
...
Shiny app is loaded. There is a second scenario where the interactive heatmap
