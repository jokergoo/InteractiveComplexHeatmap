...
boxes. You can check `htShinyExample(16)` where the size of the heatmap box is
...
with heatmaps by `htShinyExample(22)`.
...
You can also run the following example simply by `htShinyExample(20)`.
...
`htShinyExample(21)` gives another scenario where the output needs to be
...
IDs and UniProt IDs). Run `htShinyExample(21)` to see how this is implemented.
